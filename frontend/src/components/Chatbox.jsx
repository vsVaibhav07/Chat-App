...
const Chatbox = () => {
  ...
  const handleMessageSend = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/message/send/${receiverId}`, { message, receiverId }, { withCredentials: true });
      setMessage("");
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="h-[94vh] w-full sm:w-[60%] flex flex-col rounded-xl overflow-hidden bg-gray-800 shadow-2xl">
      <div className="flex items-center gap-2 sm:gap-4 p-4 border-b bg-gray-700">
        ...
        <div>
          <h2 className="text-lg text-white font-semibold tracking-wide">{selectedUser?.fullName}</h2>
          <p className={`text-sm ${isOnline(receiverId) ? "text-green-500" : "text-gray-500"}`}>
            {isOnline(receiverId) ? "Online" : ""}
          </p>
        </div>
        <div className="flex-grow flex justify-end">
          <button onClick={startCall} className="btn btn-sm btn-ghost text-white text-2xl" title="Start video call">
            <MdVideoCall />
          </button>
      ...
      <form onSubmit={handleMessageSend} className="flex items-center gap-0.5 sm:gap-2 px-0.5 sm:px-4 py-3 border-t bg-gray-700/80 backdrop-blur">
        <input
          type="text"
